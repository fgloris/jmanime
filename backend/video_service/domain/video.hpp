...
struct VideoFormat {
  ...
  std::string video_codec;   // video codec like "h264", "h265". "libx264" is a library name that performs h264 encoding.
  std::string audio_codec;   // video codec like "aac"
  std::string debug(){
    return std::format("format:{},video_codec:{},audio_codec:{},width:{},height:{},resolution:{},crf:{},bitrate:{},pix_fmt:{}",
      format,video_codec,audio_codec,width,height,width*height,crf,bitrate,av_get_pix_fmt_name(pix_fmt));
  }
};
