...
- **SSL （Secure Sockets Layer）加密**：在任意一个协议中均可使用。SSL 握手过程如下。
  ...
  如何正确关闭SSL连接？
  需要先两侧互发close_notify，或者一侧发送之后不等待另一侧就关闭连接。
...
## review 1:
  以下代码会发生异常，即使经过了ssl关闭的修正。
  代码在结束时会发生Segmentation Fault。
  问题在于某个库的RAII的实现有问题。
  mysqlconn和boost都使用了openssl库，他们对openssl的context的释放存在double free问题。
  发现把对repository->findByEmail(email)调到后面可以解决这个问题。
  所以可以用std::shared_ptr或者unique_ptr来包装ssl的context，调用reset()手动释放。
  以下是没有经过处理的错误代码。
  ```cpp
